...
- Registration can select `Human Interviewer` or `AI Interviewer`. Both modes capture candidate years of experience, a technology-specific target role, starting difficulty level from 1 to 5, and max question count. Human mode keeps AV mode selection on the follow-up setup step so the registration page stays focused on candidate/interview attributes.
