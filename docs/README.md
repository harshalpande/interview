...
- `Standard Multiple Questions` keeps the existing independent-question flow. `Banyan Style` uses one evolving Java/Python challenge: each successful frozen level is preserved as hidden evidence, the next level extends the same problem with additional requirements/assertions, and a failed frozen level ends the AI interview instead of generating another level.
