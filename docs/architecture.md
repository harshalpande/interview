...
- Evaluation style controls AI progression. `Standard Multiple Questions` keeps independent question tabs. `Banyan Style` keeps a single evolving Java/Python challenge visible to the candidate, preserves each passed level as hidden submitted evidence, asks the AI service to extend the same challenge with the next requirement/assertions, and blocks further generation if the latest level did not pass. Banyan Level 1 is selected from a curated backend problem-family pool, and the selected family marker is preserved in question concepts so Level 2+ keeps the same business domain instead of drifting into generic string/array/list questions. Banyan generation is experience-aware across deterministic bands: 1-3, 4-6, 7-10, 11-15, 16-20, and 20+ years. Each band has bounded Level 1 scope and level-by-level growth rules so genuine candidates are tested fairly for their experience instead of receiving an overloaded first problem.
